			...
			<div class="col-md-8">
				<div class="main-container" :class="{ loading: isLoading }">
					...
					>
						...
						<div
							class="step"
							v-if="lowest_hierarchy !== 'State' && lowest_hierarchy !== 'District'"
						...
						<div
							class="step"
							v-if="
								lowest_hierarchy !== 'State' &&
								lowest_hierarchy !== 'District' &&
								lowest_hierarchy !== 'Block'
							"
						...
						<div class="mb-3">
							<div class="form-check">
								...
								<label
									...
									:for="`select-all-states`"
									@click.prevent="toggleAllStates"
								>
							...
							>
								<div class="form-check">
									...
									<label
										class="form-check-label"
										:for="`state-${state.id}`"
										@click.prevent="toggleState(state.id)"
									>
										{{ state.name }}
									</label>
								</div>
					...
					<div v-if="currentStep === 2">
						...
						<div class="mb-3">
							<div class="form-check">
								...
								<label
									class="form-check-label"
									:for="`select-all-districts`"
									@click.prevent="toggleAllDistricts"
								>
						...
						>
							<div class="state-header mb-2">
								...
								<div class="form-check">
									<input
										type="checkbox"
										:id="`select-all-districts-${stateId}`"
									...
									<label
										class="form-check-label"
										:for="`select-all-districts-${stateId}`"
										@click.prevent="toggleAllDistrictsForState(stateId)"
									>
										Select All {{ __("Districts") }}
									</label>
								</div>
							...
							<div class="checkbox-container">
								<div
									class="checkbox-item"
									v-for="district in getDistrictsForState(stateId)"
									:key="district.id"
								>
									<div class="form-check">
										<input
											...
											class="form-check-input"
											:disabled="
												read_only || isDistrictPreserved(district.id)
											"
										/>
										<label
											class="form-check-label"
											:for="`district-${district.id}`"
											@click.prevent="toggleDistrict(district.id)"
										>
											{{ district.name }}
										</label>
									</div>
					...
					<div v-if="currentStep === 3">
						...
						<div
							v-for="districtId in selectedDistricts"
							:key="districtId"
						...
						>
							<div class="district-header mb-2">
								<div class="hierarchy-path">
									<span class="path-item">{{
										getStateName(getDistrictState(districtId))
									}}</span>
								...
								<div class="form-check">
									...
									<label
										class="form-check-label"
										:for="`select-all-blocks-${districtId}`"
									>
										Select All {{ __("Blocks") }}
									</label>
								</div>
							...
							<div class="checkbox-container">
								<div
									class="checkbox-item"
									v-for="block in getBlocksForDistrict(districtId)"
									:key="block.id"
								>
									<div class="form-check">
										<input
											...
											@change="updateGramPanchayats"
											:disabled="read_only || isBlockPreserved(block.id)"
										/>
										<label
											class="form-check-label"
											:for="`block-${block.id}`"
											@click.prevent="toggleBlock(block.id)"
										>
											{{ block.name }}
										</label>
									</div>
					...
					<div v-if="currentStep === 4">
						...
						<div class="mb-3">
							<div class="form-check">
								<input
									...
									type="checkbox"
									:id="`select-all-gps`"
									:checked="allGramPanchayatsSelected"
						...
						<div
							v-for="blockId in selectedBlocks"
							:key="blockId"
						...
						>
							<div class="block-header mb-2">
								<div class="hierarchy-path">
									<span class="path-item">{{
										getStateName(getBlockState(blockId))
									}}</span>
									<span class="path-separator">></span>
									<span class="path-item">{{
										getDistrictName(getBlockDistrict(blockId))
									}}</span>
									<span class="path-separator">></span>
							...
							<div class="checkbox-container">
								<div
									class="checkbox-item"
									v-for="gp in getGramPanchayatsForBlock(blockId)"
									:key="gp.id"
								>
									<div class="form-check">
										<input
											...
											:id="`gp-${gp.id}`"
											:value="gp.id"
											v-model="selectedGramPanchayats"
											@change="updateVillages"
											:disabled="
												read_only || isGramPanchayatPreserved(gp.id)
											"
										/>
										<label
											class="form-check-label"
											:for="`gp-${gp.id}`"
											@click.prevent="toggleGramPanchayat(gp.id)"
										>
											{{ gp.name }}
										</label>
									</div>
					...
					<div v-if="currentStep === 5">
						...
						<div class="mb-3">
							<div class="form-check">
								<input
									class="form-check-input"
									type="checkbox"
									:id="`select-all-villages`"
									:checked="allVillagesSelected"
						...
						<div
							v-for="gpId in selectedGramPanchayats"
							:key="gpId"
						...
						>
							<div class="gp-header mb-2">
								<div class="hierarchy-path">
									<span class="path-item">{{
										getStateName(getGPState(gpId))
									}}</span>
									<span class="path-separator">></span>
									<span class="path-item">{{
										getDistrictName(getGPDistrict(gpId))
									}}</span>
									<span class="path-separator">></span>
									<span class="path-item">{{
										getBlockName(getGPBlock(gpId))
									}}</span>
									<span class="path-separator">></span>
							...
							<div class="checkbox-container">
								<div
									class="checkbox-item"
									v-for="village in getVillagesForGP(gpId)"
									:key="village.id"
								>
									<div class="form-check">
										<input
										...
										<label
											...
											@click.prevent="toggleVillage(village.id)"
										>
											{{ village.name }}
										</label>
									</div>
					...
					<div class="geography-tree">
						...
						>
							<div
								class="tree-content"
								@click="
									lowest_hierarchy !== 'State' && toggleStateExpansion(stateId)
								"
							>
								<span
									class="tree-icon toggle-icon"
									v-if="lowest_hierarchy !== 'State'"
							...
							>
								<div
									...
									v-show="selectedDistricts.includes(district.id)"
								>
									<div
										class="tree-content"
										@click="
									...
									>
										<span
											class="tree-icon toggle-icon"
											v-if="lowest_hierarchy !== 'District'"
									...
									>
										<div
											v-for="block in getBlocksForDistrict(district.id)"
											:key="block.id"
											class="tree-item block-item"
											v-show="selectedBlocks.includes(block.id)"
										>
											<div
												class="tree-content"
												@click="
											...
											>
												...
												>
													({{
														getSelectedGramPanchayatsForBlock(block.id)
															.length
													}})
												</span>
											</div>
											...
											>
												<div
													v-for="gp in getGramPanchayatsForBlock(
														block.id
													)"
													:key="gp.id"
												...
												>
													...
													>
														...
														>
															({{
																getSelectedVillagesForGP(gp.id)
																	.length
															}})
														</span>
													</div>
													<div
														...
														v-show="
															isGPExpanded(gp.id) &&
															lowest_hierarchy !== 'Gram Panchayat'
														"
													>
														<div
															v-for="village in getVillagesForGP(
																gp.id
															)"
															:key="village.id"
															class="tree-item village-item"
															v-show="
																selectedVillages.includes(
																	village.id
																)
															"
														>
...
const loadExistingData = async () => {
	if (props.existing_data.length > 0) {
		...
		props.existing_data.forEach((detail) => {
			...
			if (detail.gram_panchayat && detail.gram_panchayat.trim())
			...
			try {
				...
				if (doc) {
					...
					if (doc.geography_details) {
						...
						doc.geography_details.forEach((detail) => {
							...
							if (detail.gram_panchayat && detail.gram_panchayat.trim())
								gpSet.add(detail.gram_panchayat);
							if (detail.village && detail.village.trim())
				...
				if (props.preserve_data?.geography_details) {
					props.preserve_data.geography_details.forEach((detail) => {
						...
						if (
							detail.district &&
							!selectedDistricts.value.includes(detail.district)
						) {
						...
						if (
							detail.gram_panchayat &&
							!selectedGramPanchayats.value.includes(detail.gram_panchayat)
						) {
...
const goNext = () => {
	...
	const canProceed = (() => {
		switch (currentStep.value) {
			case 1:
				return selectedStates.value.length > 0;
			case 2:
				return selectedDistricts.value.length > 0;
			case 3:
				return selectedBlocks.value.length > 0;
			case 4:
				return selectedGramPanchayats.value.length > 0;
			default:
				return true;
		}
	})();
...
const saveSelection = async () => {
	...
	selectedVillages.value.forEach((villageId) => {
		...
		if (village) {
			...
			selectionMap.set(
				`${village.state}_${village.district}_${village.block}_${village.gram_panchayat}_${village.id}`,
				{
					state: { id: state.id, name: state.name, code: state.code },
					district: {
						id: district.id,
...
const getSelectedVillagesForGP = (gpId) => {
	return getVillagesForGP(gpId).filter((village) => selectedVillages.value.includes(village.id));
};
...
const isAllDistrictsSelectedForState = (stateId) => {
	...
	return (
		...
		stateDistricts.every((district) => selectedDistricts.value.includes(district.id))
	);
};
...
const isAllBlocksSelectedForDistrict = (districtId) => {
	...
	return (
		...
		districtBlocks.every((block) => selectedBlocks.value.includes(block.id))
	);
};
...
const isAllGramPanchayatsSelectedForBlock = (blockId) => {
	...
	return (
		blockGPs.length > 0 && blockGPs.every((gp) => selectedGramPanchayats.value.includes(gp.id))
	);
};
...
const isAllVillagesSelectedForGP = (gpId) => {
	...
	return (
		gpVillages.length > 0 &&
		gpVillages.every((village) => selectedVillages.value.includes(village.id))
	);
};
	...
	switch (currentStep.value) {
		...
		case 2:
			...
			if (statesWithoutDistricts.length > 0) {
				...
				validation.message =
					statesWithoutDistricts.length === 1
		...
		case 3:
			...
			if (districtsWithoutBlocks.length > 0) {
				...
				validation.message =
					districtsWithoutBlocks.length === 1
...
defineExpose({
	...
	getGramPanchayatsForBlock,
	getVillagesForGP,
	getSelectedDistrictsForState,
	getSelectedBlocksForDistrict,
	getSelectedGramPanchayatsForBlock,
	getSelectedVillagesForGP,
	getSelectedBlocksForStateRecursive,
	getSelectedGPsForStateRecursive,
	getDistrictState,
	getBlockState,
	getBlockDistrict,
	getGPState,
	...
	toggleGramPanchayat,
	toggleVillage,
	isAllDistrictsSelectedForState,
	toggleAllDistrictsForState,
	isAllBlocksSelectedForDistrict,
	toggleAllBlocksForDistrict,
	isAllGramPanchayatsSelectedForBlock,
	toggleAllGramPanchayatsForBlock,
	isAllVillagesSelectedForGP,
	toggleAllVillagesForGP,
	toggleAllStates,
	toggleAllDistricts,
	toggleAllBlocks,
