	...
	if allow == "Zone":
		...
		if not exist:
			zone_perm = frappe.get_doc(
				{"doctype": "User Permission", "allow": "Zone", "for_value": for_value, "user": user}
			)
			zone_perm.insert(ignore_permissions=True)
			#
	elif allow == "Company Profile":
		state_exist = frappe.db.exists(
			"User Permission", {"allow": "Company Profile", "for_value": for_value, "user": user}
		)
		...
		state_exist = frappe.db.exists(
			"User Permission", {"allow": "State", "for_value": for_value, "user": user}
		)
		if is_zone_mandatory:
			zone_value = frappe.db.get_value("State", for_value, "zone")
			zone_exist = frappe.db.exists(
				"User Permission", {"allow": "Zone", "for_value": zone_value, "user": user}
			)
			if not zone_exist:
				zone_perm = frappe.get_doc(
					{"doctype": "User Permission", "allow": "Zone", "for_value": zone_value, "user": user}
				)
				zone_perm.insert(ignore_permissions=True)
		if not state_exist:
			state_perm = frappe.get_doc(
				{"doctype": "User Permission", "allow": "State", "for_value": for_value, "user": user}
			)
			state_perm.insert(ignore_permissions=True)
	elif allow == "District":
		...
		if state_value:
			if is_zone_mandatory:
				...
				if not zone_exist:
					...
					zone_perm.insert(ignore_permissions=True)
			state_exist = frappe.db.exists(
				"User Permission", {"allow": "State", "for_value": state_value, "user": user}
			)
			dist_exist = frappe.db.exists(
				"User Permission", {"allow": "District", "for_value": for_value, "user": user}
			)
			if not state_exist:
				state_perm = frappe.get_doc(
					{"doctype": "User Permission", "allow": "State", "for_value": state_value, "user": user}
				)
				state_perm.insert(ignore_permissions=True)
			if not dist_exist:
				dist_perm = frappe.get_doc(
					{"doctype": "User Permission", "allow": "District", "for_value": for_value, "user": user}
				)
		...
		if state_value:
			...
			dist_exist = frappe.db.exists(
				"User Permission", {"allow": "District", "for_value": dist_value, "user": user}
			)
			center_exist = frappe.db.exists(
				"User Permission", {"allow": "Center", "for_value": for_value, "user": user}
			)
			if not state_exist:
				state_perm = frappe.get_doc(
					{"doctype": "User Permission", "allow": "State", "for_value": state_value, "user": user}
			...
			if not dist_exist:
				dist_perm = frappe.get_doc(
					{"doctype": "User Permission", "allow": "District", "for_value": dist_value, "user": user}
				)
				dist_perm.insert(ignore_permissions=True)
			if not center_exist:
				center_perm = frappe.get_doc(
					{"doctype": "User Permission", "allow": "Center", "for_value": for_value, "user": user}
				)
		...
		if state_value and dist_value:
			...
			block_exist = frappe.db.exists(
				"User Permission", {"allow": "Block", "for_value": for_value, "user": user}
			)
			if not state_exist:
				state_perm = frappe.get_doc(
					{"doctype": "User Permission", "allow": "State", "for_value": state_value, "user": user}
			...
			if not block_exist:
				block_perm = frappe.get_doc(
					{"doctype": "User Permission", "allow": "Block", "for_value": for_value, "user": user}
				)
		...
		if state_value and dist_value and block_value:
			...
			block_exist = frappe.db.exists(
				"User Permission", {"allow": "Block", "for_value": block_value, "user": user}
			)
			grampanchayat_exist = frappe.db.exists(
				"User Permission", {"allow": "Grampanchayat", "for_value": for_value, "user": user}
			)
			if not state_exist:
				state_perm = frappe.get_doc(
					{"doctype": "User Permission", "allow": "State", "for_value": state_value, "user": user}
			...
			if not block_exist:
				block_perm = frappe.get_doc(
					{"doctype": "User Permission", "allow": "Block", "for_value": block_value, "user": user}
				)
				block_perm.insert(ignore_permissions=True)
		...
		if state_value and dist_value and block_value and gp_value:
			...
			gp_exist = frappe.db.exists(
				"User Permission", {"allow": "Grampanchayat", "for_value": gp_value, "user": user}
			)
			village_exist = frappe.db.exists(
				"User Permission", {"allow": "Village", "for_value": for_value, "user": user}
			)
			if not state_exist:
				state_perm = frappe.get_doc(
					{"doctype": "User Permission", "allow": "State", "for_value": state_value, "user": user}
			...
			if not village_exist:
				village_perm = frappe.get_doc(
					{"doctype": "User Permission", "allow": "Village", "for_value": for_value, "user": user}
				)
...
def get_roles_and_permissions_by_profile(role_profile):
	...
	roles = frappe.get_all("Has Role", filters={"parent": role_profile}, fields=["role"], pluck="role")
	...
	doctypes_with_custom_perms = frappe.get_all(
		"Custom DocPerm", pluck="parent", filters={"role": ["in", roles]}, distinct=True
	)
	custom_perms = frappe.get_all("Custom DocPerm", fields=permission_fields, filters={"role": ["in", roles]})
	perms = frappe.get_all(
		"DocPerm",
		fields=permission_fields,
		filters={"role": ["in", roles], "parent": ["not in", doctypes_with_custom_perms]},
	)
	...
	result = {
		"role_profile": role_profile,
		"roles_and_permissions": sorted(
			[{field: role[field] for field in permission_fields} for role in custom_perms],
			key=lambda x: (
